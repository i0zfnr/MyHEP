...
@section('title', 'Edit Pengumuman Scholarship')
...
<style>
    ...
    .btn-danger { border-color: #fca5a5 !important; color: #b91c1c !important; background: #fef2f2 !important; }
    .error {
        margin-bottom: 12px;
        background: #fef2f2;
    ...
    .existing-poster img { width: 100%; max-height: 280px; object-fit: contain; display: block; background: #f8f2ea; }
    .existing-poster-actions {
            ...
            <div class="body">
                    ...
                    <div>
                        ...
                        <select id="type" name="type" required>
                            @foreach(['scholarship','welfare','general'] as $t)
                                <option value="{{ $t }}" {{ old('type', $announcement->type) === $t ? 'selected' : '' }}>{{ $t }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div>
                    ...
                    @if($announcement->poster_image)
                        <div id="existingPosterWrap">
                            <div class="existing-poster">
                                <img src="{{ Storage::disk('public')->url($announcement->poster_image) }}" alt="Poster semasa">
                            </div>
                            <div class="existing-poster-actions">
                                <span style="font-size:12px;color:#7b6757;">{{ __('Poster semasa') }}</span>
                                <label style="display:inline-flex;align-items:center;gap:6px;font-size:13px;cursor:pointer;margin:0;">
                                    <input type="checkbox" name="remove_poster" value="1" id="removePosterCheck" {{ old('remove_poster') ? 'checked' : '' }} style="width:auto;">
                                    {{ __('Padam poster ini') }}
                                </label>
                                <span style="font-size:12px;color:#9a8476;">{{ __('atau pilih gambar baru di bawah untuk menggantikan') }}</span>
                            </div>
                        </div>
                    @endif
                    ...
                    <div class="poster-zone" id="posterZone" style="{{ $announcement->poster_image ? 'margin-top:12px;' : '' }}">
                        ...
                        <div class="poster-zone__text">{{ $announcement->poster_image ? __('Pilih gambar baru untuk menggantikan') : __('Klik atau seret gambar poster di sini') }}</div>
                ...
                <div class="grid grid-2">
                    <div>
                        ...
                        <input id="contact_email" type="email" name="contact_email" value="{{ old('contact_email', $announcement->contact_email) }}" placeholder="contoh@edu.my">
                    ...
                    <div>
                        ...
                        <input id="contact_phone" type="tel" name="contact_phone" value="{{ old('contact_phone', $announcement->contact_phone) }}" placeholder="+60123456789">
                ...
                <div class="grid grid-2">
                    <div>
                        ...
                        <input id="link_url" type="url" name="link_url" value="{{ old('link_url', $announcement->link_url) }}" placeholder="https://example.com">
                    </div>
                    <div>
