...
@section('title', 'Tambah Rekod Scholarship')
...
@section('header')
    <h2 style="margin:0;font-size:1.1rem;font-weight:700;color:#2d1f14;">{{ __('Tambah Rekod Scholarship') }}</h2>
@endsection
            ...
            <div class="body">
                ...
                <div class="grid grid-2" style="margin-top:12px;">
                    <div>
                        ...
                        <input id="provider_name" type="text" name="provider_name" value="{{ old('provider_name') }}" placeholder="{{ __('Contoh: JPA / MARA') }}">
                    </div>
                    <div></div>
                </div>
                    ...
                    <div>
                        ...
                        <select id="type" name="type" required>
                            @foreach(['scholarship','welfare','sponsorship','none'] as $type)
                                <option value="{{ $type }}" {{ old('type','none') === $type ? 'selected' : '' }}>{{ $type }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                    ...
                    <div>
                        ...
                        <select id="status" name="status" required>
                            @foreach(['pending','confirmed','rejected'] as $status)
                                <option value="{{ $status }}" {{ old('status','pending') === $status ? 'selected' : '' }}>{{ __($status) }}</option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div style="margin-top:12px;">
                    ...
                    <input id="proof_file" type="text" name="proof_file" value="{{ old('proof_file') }}" placeholder="{{ __('Contoh: uploads/proof/file.pdf') }}">
                </div>
            </div>
        </div>

        <div class="actions">
            <button class="btn btn-primary" type="submit">{{ __('Simpan Rekod') }}</button>
