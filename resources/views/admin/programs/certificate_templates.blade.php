...
<main class="cert-template-page">
    ...
    <form id="certificateTemplateEditor" method="post" action="{{ route('admin.program-certificate-templates.store') }}" enctype="multipart/form-data" class="cert-editor">
        ...
        <section class="cert-editor-panel">
            ...
            <button id="saveCertificateTemplate" class="btn btn-primary cert-save-btn" type="submit" disabled>{{ __('Save Template') }}</button>
        </section>
...
.cert-ai-btn{width:100%;justify-content:center;background:linear-gradient(135deg,#7c58bd,#b99150);color:#fff;border:0}.cert-ai-btn:disabled{opacity:.6;cursor:wait}.cert-ai-status{min-height:1.25rem;font-size:.84rem;font-weight:750;color:var(--text-muted,#746b62)}.cert-ai-status.success{color:#047857}.cert-ai-status.error{color:#b91c1c}.cert-drag-field{width:auto!important;min-width:110px;max-width:180px;transform:translateX(-50%);text-align:center;white-space:nowrap;overflow:hidden}
