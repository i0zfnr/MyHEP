...
@endphp
<style>
    .admin-ai-fab { position:fixed; right:24px; bottom:24px; z-index:1060; width:58px; height:58px; border:1px solid color-mix(in srgb,var(--primary) 58%,var(--border)); border-radius:18px; background:linear-gradient(145deg,color-mix(in srgb,var(--primary) 88%,#fff),color-mix(in srgb,var(--primary) 65%,#37261b)); color:#fff; box-shadow:0 18px 38px rgba(35,24,16,.28); display:grid; place-items:center; cursor:pointer; transition:transform .18s ease,box-shadow .18s ease; }
    ...
    .admin-ai-popover { position:fixed; right:24px; bottom:94px; z-index:1061; width:min(390px,calc(100vw - 28px)); height:min(590px,calc(100vh - 130px)); display:grid; grid-template-rows:auto 1fr auto; border:1px solid color-mix(in srgb,var(--primary) 35%,var(--border)); border-radius:22px; overflow:hidden; background:color-mix(in srgb,var(--surface) 94%,transparent); box-shadow:0 28px 70px rgba(20,14,10,.34); backdrop-filter:blur(22px) saturate(135%); opacity:0; visibility:hidden; transform:translateY(14px) scale(.98); transform-origin:bottom right; transition:.2s ease; }
    ...
    .admin-ai-pop-icon { width:38px; height:38px; border-radius:12px; display:grid; place-items:center; background:color-mix(in srgb,var(--primary) 20%,var(--surface)); color:var(--primary); font-size:18px; }
    ...
    .admin-ai-pop-send { border:0; border-radius:13px; background:var(--primary); color:#fff; font-weight:900; cursor:pointer; }
    .admin-ai-pop-send:disabled { opacity:.55; cursor:wait; }
    .admin-ai-pop-link { display:inline-block; margin-top:8px; color:var(--primary); font-size:.74rem; font-weight:800; text-decoration:none; }
    @media(max-width:640px){.admin-ai-fab{right:14px;bottom:82px}.admin-ai-popover{right:14px;bottom:150px;height:min(520px,calc(100vh - 175px))}}
</style>
...
<button type="button" class="admin-ai-fab" id="adminAiFab" aria-controls="adminAiPopover" aria-expanded="false" aria-label="{{ __('Open AI Helper') }}">
    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 3l1.7 4.8L18.5 9.5l-4.8 1.7L12 16l-1.7-4.8-4.8-1.7 4.8-1.7L12 3z"/><path d="M19 15l.8 2.2L22 18l-2.2.8L19 21l-.8-2.2L16 18l2.2-.8L19 15z"/></svg>
...
<section class="admin-ai-popover" id="adminAiPopover" aria-hidden="true">
    <header class="admin-ai-pop-head">
        <div class="admin-ai-pop-title"><span class="admin-ai-pop-icon">✦</span><span><strong>{{ __('Admin AI Helper') }}</strong><small>{{ strtoupper($floatingAiProvider) }} · {{ $floatingAiModel }}</small></span></div>
...
(() => {
    ...
    const toggle=(open)=>{panel.classList.toggle('is-open',open);panel.setAttribute('aria-hidden',open?'false':'true');fab.setAttribute('aria-expanded',open?'true':'false');if(open)input.focus()};
