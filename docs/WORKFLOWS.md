...
The director first decides whether the program provides certificates. Programs may award points without a certificate. When a PDF template is uploaded, Gemini maps the existing recipient Name and IC regions. The administrator reviews those regions, then the server creates a private 300-DPI cleaned master with only the old recipient text inpainted away. The original PDF is retained privately for recovery, while FPDI reuses the cleaned master for every recipient and automatically reduces the font size for long names. Existing templates without a cleaned master are upgraded on their next generation attempt. Eligible internal students are linked by matric number, bulk processing runs through the database queue, and students see Pending/Generating/Ready/Failed status before downloading a ready certificate. **Test & Preview** generates one certificate synchronously so the administrator can visually approve it before starting the bulk queue.
