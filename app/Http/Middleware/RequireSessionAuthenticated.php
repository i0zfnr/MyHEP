...
class RequireSessionAuthenticated
{
    public function handle(Request $request, Closure $next): Response
    {
        ...
        $isStaffOverride = $authUser['role'] === 'admin' && (bool) ($authUser['staff_override'] ?? false);
        if (!$account || ($authUser['role'] === 'admin' && !$isStaffOverride && ($account->role ?? null) !== ($authUser['admin_role'] ?? null))) {
