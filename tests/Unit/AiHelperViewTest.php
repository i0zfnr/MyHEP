...
class AiHelperViewTest extends TestCase
{
    public function test_database_table_names_are_not_loaded_as_translation_groups(): void
    {
        ...
        $this->assertStringContainsString('Scope: students, scholarships, offenses, applications', $view);
    }

    public function test_admin_ai_helper_uses_accent_tokens_and_hides_native_file_control(): void
    {
        $view = file_get_contents(__DIR__ . '/../../resources/views/admin/ai_helper/index.blade.php');
        $styles = file_get_contents(__DIR__.'/../../resources/css/admin-modules.css');

        $this->assertStringContainsString('var(--se-primary-button-start)', $styles);
        $this->assertStringContainsString('var(--se-primary-soft)', $styles);
        $this->assertStringContainsString('.ai-upload-drop input { display:none !important;', $styles);
        $this->assertStringContainsString('body:not([data-theme="dark"]) .ai-compose-frame .ai-compose-row {', $styles);
        $this->assertStringContainsString('body:not([data-theme="dark"]) .ai-admin .msg.user pre {', $styles);
        $this->assertStringContainsString('body:not([data-theme="dark"]) .ai-format-pill {', $styles);
    }
    ...
    public function test_shared_ai_view_has_a_two_column_student_composer(): void
    {
        ...
        $this->assertStringContainsString('.ai-admin--student .ai-compose-row { grid-template-columns:minmax(0,1fr) 46px !important;', $styles);
    }
    ...
    public function test_lecturer_ai_view_uses_its_own_routes_and_attachment_composer(): void
    {
        ...
        $this->assertStringContainsString('bottom:calc(100% + 8px)', $styles);
        $this->assertStringContainsString('<div class="ai-compose-frame">', $view);
        $this->assertStringContainsString('flex:0 0 104px', $styles);
        $this->assertStringContainsString('min-width:25px !important', $styles);
        ...
        $this->assertStringContainsString('const sentAttachments = [...selectedAttachments];', $view);
        $this->assertStringContainsString('clearComposerAttachments();', $view);
        $this->assertStringContainsString("addMessage('user', message, '', sentAttachments)", $view);
        $this->assertStringContainsString('.ai-admin--lecturer .ai-compose-row', $styles);
    }
    ...
    public function test_admin_quick_actions_drawer_follows_system_accent_tokens(): void
    {
        ...
        $this->assertStringContainsString('.ai-admin--admin > aside.ai-panel {', $styles);
        $this->assertStringContainsString('.ai-admin--admin .ops-card {', $styles);
        $this->assertStringContainsString('.ai-admin--admin .task-btn:hover', $styles);
        $this->assertStringContainsString('background:var(--se-primary-soft) !important;', $styles);
        $this->assertStringContainsString('border-color:var(--se-primary) !important;', $styles);
    }
    ...
    public function test_admin_ai_workspace_uses_available_layout_height_without_page_scroll(): void
    {
        ...
        $this->assertStringContainsString('body.admin-ai-helper-page .main-scroll-viewport { overflow:hidden !important; }', $styles);
        $this->assertStringContainsString('body.admin-ai-helper-page .main-scroll-inner { height:100%; min-height:0; overflow:hidden; }', $styles);
        $this->assertStringContainsString('body.admin-ai-helper-page .page-body { flex:1 1 auto; height:auto !important; min-height:0;', $styles);
        $this->assertStringContainsString('min-height:0 !important; height:100% !important;', $styles);
        $this->assertStringNotContainsString('height:calc(100vh - var(--topbar-h))', $styles);
    }
    ...
    public function test_mobile_ai_workspace_keeps_composer_in_flow_and_messages_scrollable(): void
    {
        ...
        $this->assertStringContainsString('.ai-chat-log { min-height:0 !important; height:auto !important; overflow-y:auto !important;', $styles);
        $this->assertStringContainsString('.ai-compose { position:relative !important; bottom:auto !important;', $styles);
        $this->assertStringContainsString('body.admin-ai-helper-page .page-body { padding-bottom:calc(4.7rem + env(safe-area-inset-bottom,0px)) !important;', $styles);
        $this->assertStringContainsString('body.admin-ai-helper-page .ai-compose {', $styles);
        $this->assertStringContainsString('position:fixed !important;', $styles);
        $this->assertStringContainsString('.ai-hint { display:none; }', $styles);
        $this->assertStringContainsString('.ai-message-action span { display:none; }', $styles);
        $this->assertStringContainsString('.ai-admin--admin .ai-compose-row { grid-template-columns:minmax(0,1fr) 42px !important;', $styles);
        $this->assertStringContainsString('.ai-format-pill { display:none !important; }', $styles);
    }
    ...
    public function test_student_ai_history_and_chat_bubbles_use_accent_tokens(): void
    {
        ...
        $this->assertStringContainsString('transform:translateX(calc(100% + 32px))', $styles);
        $this->assertStringContainsString('opacity:0; visibility:hidden; pointer-events:none;', $styles);
        $this->assertStringContainsString('.ai-history-panel.is-open { right:16px; left:auto; opacity:1; visibility:visible;', $styles);
        $this->assertStringContainsString('.ai-admin > .ai-panel:first-child { position:relative;', $styles);
        ...
        $this->assertStringContainsString('grid-template-rows:auto minmax(390px,1fr) auto auto !important;', $styles);
        $this->assertStringContainsString('border:1px solid color-mix(in srgb,var(--se-primary) 42%,var(--border)); border-radius:999px;', $styles);
        ...
        $this->assertStringContainsString('.ai-history-brand { width:100%;', $styles);
        $this->assertStringContainsString('id="aiConfirmDialog"', $view);
        $this->assertStringContainsString('const requestConfirmation =', $view);
        $this->assertStringContainsString('background:transparent; opacity:0; visibility:hidden;', $styles);
        $this->assertStringContainsString('height:min(760px,calc(100% - 32px));', $styles);
        $this->assertStringContainsString('border-radius:20px; background:#1e1e1e;', $styles);
        $this->assertStringContainsString('data-ai-session-key="myhep.ai.active.', $view);
        ...
        $this->assertStringContainsString("key.startsWith('myhep.ai.active.')", $app);
        ...
        $this->assertStringContainsString('.ai-admin .msg.user {', $styles);
        $this->assertStringContainsString('var(--se-primary-button-start)', $styles);
        $this->assertStringContainsString('.ai-admin .msg.ai {', $styles);
        $this->assertStringContainsString('var(--se-primary-soft)', $styles);
        $this->assertStringContainsString('body.student-bottom-nav-eligible.admin-ai-helper-page .page-body { padding-bottom:calc(10.8rem + env(safe-area-inset-bottom,0px)) !important; }', $styles);
        $this->assertStringContainsString('bottom:calc(6.65rem + env(safe-area-inset-bottom,0px)) !important;', $styles);
    ...
    public function test_reply_actions_sit_below_the_latest_ai_response(): void
    {
        ...
        $this->assertStringContainsString('.ai-message-actions { display:flex;', $styles);
        ...
        $this->assertStringContainsString('.ai-admin .msg.ai.is-conversation', $styles);
        $this->assertStringContainsString('.ai-admin .msg.ai.is-report', $styles);
        ...
        $this->assertStringContainsString('.ai-compose-frame:has(.ai-compose-context.is-visible)', $styles);
        $this->assertStringContainsString('.ai-compose-context[hidden] { display:none !important; }', $styles);
        $this->assertStringContainsString('.ai-compose-context { display:none; height:34px;', $styles);
        $this->assertStringContainsString('.ai-compose:has(.ai-compose-context.is-visible) .ai-hint { display:none; }', $styles);
        ...
        $this->assertStringContainsString('body.ai-writing-expanded #appSidebar', $styles);
    ...
    public function test_chat_scrollbars_are_visible_and_follow_the_accent(): void
    {
        ...
        $aiStyles = file_get_contents(__DIR__.'/../../resources/css/admin-modules.css');
        ...
        $this->assertStringContainsString('.ai-chat-log.can-scroll-up.can-scroll-down', $aiStyles);
    ...
    public function test_ai_replies_render_as_safe_structured_reports(): void
    {
        ...
        $this->assertStringContainsString('.msg-rich h3 {', $styles);
        $this->assertStringContainsString('@media(min-width:641px){', $styles);
        $this->assertStringContainsString('.msg.ai { max-width:min(88%,680px); }', $styles);
        $this->assertStringContainsString('.msg { padding:.72rem .82rem; font-size:.8rem;', $styles);
    }
}
